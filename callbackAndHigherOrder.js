...
each(names, printStr = (item, index) => {
...
const getUserById = (arrayOfUsers, id, callback) => {
  arrayOfUsers.forEach((name, index) => {
    if (name.id === id) {
      callback(arrayOfUsers[index]);
    }
  })
}
...
const addNumber = addingFactory(15);
